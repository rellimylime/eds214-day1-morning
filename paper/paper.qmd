...
Compound and site variables were converted to categorical factors and pivoted into long format for easier grouping and plotting. The `find_median(x)` function is applied to the grouped dataframe before applying `calc_moving_avg(dates, values, ...)` to the resulting data frame grouped by site, compound, and date.[^1]

[^1]: This was done with merged and cleaned data; the four relevant raw data files were [loaded](scripts/1_load_data.R) combined into one dataframe, this manipulation can be seen [here](scripts/2_process_data.R).
