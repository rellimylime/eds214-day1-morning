...
# Add column with site name to each dataframe
BQ1_df <- BQ1_df %>%
  mutate(site = "BQ1")
...
long <- all_sites_df %>%
  ...
  value = calc_moving_avg(long, df[[compound]], half_interval = 4, min_gap_size = 3)
    
  ggplot(df, aes(x = date, y = rolling_means, color = site, group = site)) +
    geom_line(na.rm = TRUE) +
    facet_wrap(~ compound, ncol = 1, scales = "free_y") +
    scale_linetype_manual(
      name = NULL,
      breaks = c("PRM", "BQ1", "BQ2", "BQ3"),
